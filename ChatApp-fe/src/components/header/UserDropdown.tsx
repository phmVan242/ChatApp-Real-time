...
export default function UserDropdown() {
  ...
  const loadUserInfo = async () => {
    try {
      ...
      if (!token) {
        setUserInfo({ name: "Guest User", email: "guest@example.com" });
        return;
      }
      ...
      // Lưu vào localStorage để dùng sau (cập nhật)
      ...
      // Fallback: đọc từ localStorage cũ nếu có
    ...
    <div className="relative">
      ...
      >
        <div>
          ...
          {userInfo.role && (
            <span className="mt-1 inline-block px-2 py-0.5 text-xs font-medium bg-brand-50 text-brand-700 rounded-full dark:bg-brand-500/20 dark:text-brand-400">
              {userInfo.role}
            </span>
          )}
        </div>
