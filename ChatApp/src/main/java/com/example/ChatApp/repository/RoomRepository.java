...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query("SELECT r FROM Room r WHERE r.type = 'PRIVATE' AND " +
            "EXISTS (SELECT m1 FROM r.members m1 WHERE m1.user.id = :user1Id) AND " +
            "EXISTS (SELECT m2 FROM r.members m2 WHERE m2.user.id = :user2Id) AND " +
            "SIZE(r.members) = 2")
